...
func main() {
	...
	startTime := currentDate.AddDate(0, 0, -1).Format("2006-01-02 15:04:05")
...
<footer >
	<p style="text-align: center;font-size: 14px;width: 100%;position: absolute;"><a href="https://github.com/wangtuo1224/mysql_slowlog_report">GitHub</a> <a href="https://aikbuk.com">个人技术博客</a></p>
		...
		slowlogs := make([]Slowlog,len(data.Classes))
		// 将解析后的json数据重新组装
		for k,v := range data.Classes {
			// TODO 过滤指定用户
			//if v.Metrics.Users.Value == "cloud" {
			//	break
			//}
			slowlogs[k].Ts_min = v.Ts_min
			slowlogs[k].Ts_max = v.Ts_max
			slowlogs[k].Query_count = v.Query_count
			slowlogs[k].Query = v.Examples.Query
			...
			slowlogs[k].Pct_95 = float_Pct_95
			slowlogs[k].User = v.Metrics.Users.Value
			slowlogs[k].Host = v.Metrics.Hosts.Value
			slowlogs[k].Db = v.Metrics.Dbs.Value
		}
		// 按照出现次数降序
...
`)
		...
		if *limitRecords <= len(data.Classes) {
			if err := slowquery_tpl.Execute(rend_result, slowlogs[0:*limitRecords]); err != nil {
				log.Panicf("render to html have errors->%s", err)
			}
		} else {
			if err := slowquery_tpl.Execute(rend_result, slowlogs[0:len(data.Classes)]); err !=nil {
				log.Panicf("render to html have errors->%s", err)
			}
		}
	}

	if err := util.SendEmail("请及时处理("+*tag+") MySQL慢日志"+"("+startTime+"-"+ endTime+")", rend_result.String(),""); err != nil {
